...
def get_tokenizer(args):
    ...
    cfg_name = args.tokenizer_path or 'bert-base-uncased'
    tokenizer = AutoTokenizer.from_pretrained(cfg_name, local_files_only=args.local_files_only)
    return tokenizer
...
def train(args, train_env, train_full_traj_env, val_envs, val_full_traj_envs, aug_env=None, rank=-1):
    ...
    for idx in range(start_iter, start_iter+args.iters, interval):
        ...
        ) # nss_w_weighting = max(0, (args.iters/2 - idx)/ (args.iters/2)))
        ...
        # 为了节省显存，直接复用训练中的 agent 进行验证，避免加载双倍 Qwen 模型导致 OOM
        for env_name, env in val_envs.items():
                agent.env = env
                loader = torch.utils.data.DataLoader(env, batch_size = 1)
                # Get validation distance from goal under test evaluation conditions
                ...
                if env_name in best_val:
                    if score_summary['spl'] >= best_val[env_name]['spl']:
                        ...
                        agent.save(iter, os.path.join(args.ckpt_dir, "best_%s" % (env_name)))
...
def main():
    ...
    record_file = os.path.join(args.output_dir, 'logs', 'train.txt')
    write_to_record_file("main: before build_dataset", record_file)
    train_env, train_full_traj_env, val_envs, val_full_traj_envs = build_dataset(args, rank=rank)
    write_to_record_file("main: after build_dataset", record_file)
